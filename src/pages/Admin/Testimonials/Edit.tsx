...
interface EditProps {
  testimonial: LaravelTestimonial | null;
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export function Edit({ testimonial, isOpen, onClose, onSuccess }: EditProps) {
  ...
  useEffect(() => {
    if (testimonial) {
      setDisplayName(testimonial.display_name || '');
      setMessage(testimonial.message || '');
      setSortOrder(testimonial.sort_order ?? 0);
      setIsActive(testimonial.is_active ?? true);
      setSelectedMediaId(testimonial.media_id || null);
      setSelectedMediaUrl(testimonial.media?.url || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await adminTestimonialsApi.update(testimonial.id, {
      ...
      showToast(isRtl ? 'تم تحديث رأي العميل بنجاح!' : 'Testimonial updated successfully!');
      onClose();
      if (onSuccess) onSuccess();
    } catch (err: any) {
      if (err?.status === 401) handle401Error();
      else if (err?.status === 403) showToast(isRtl ? 'ليس لديك صلاحية لتعديل رأي العميل.' : 'Unauthorized action.', 'danger');
      else if (err?.status === 404) showToast(isRtl ? 'هذا الرأي غير موجود بالسيرفر.' : 'Testimonial not found.', 'danger');
      else if (err?.status === 422 && err?.errors) setValidationErrors(err.errors);
      else showToast(err?.message || (isRtl ? 'تعذر تحديث رأي العميل' : 'Failed to update testimonial'), 'danger');
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">
                  {isRtl ? 'تعديل رأي العميل' : 'Edit Testimonial'}
                </h3>
                <p className="text-xs text-slate-400">ID: #{testimonial.id}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                placeholder={isRtl ? 'اكتب نص رأي العميل...' : 'Write client testimonial message...'}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
                  {isRtl ? 'ترتيب الظهور (sort_order)' : 'Sort Order'}
                </label>
                <div className="relative">
                  <input
                    type="number"
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
              ...
              >
                ...
                <span>{isRtl ? 'تحديث الرأي' : 'Update Testimonial'}</span>
              </button>
            </div>
          </form>
